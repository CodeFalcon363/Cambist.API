...
namespace Cambist.Api.Controllers
{
    ...
    public class CurrencyController : ControllerBase
    {
        ...
        // GET: api/Currency
        [HttpGet]
        public async Task<ActionResult<PagedResponse<IEnumerable<CurrencyResponse>>>> GetCurrencies(
        ...
        [HttpGet("{code}")]
        public async Task<ActionResult<ApiResponse<CurrencyResponse>>> GetCurrency(string code)
        {
            var currency = await _currency.GetByCodeAsync(code);
            if (!currency.Success) return NotFound(currency);
            return Ok(currency);
        }
    }
}
